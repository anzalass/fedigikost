...
export default function EditBarang({ close, setClose, idBarang }) {
    ...
    const UpdatePengadaan = async () => {
        try {
            const result = await axios.put("http://127.0.0.1:8000/api/updatePengadaan/" + idBarang, data);
            console.log(result);
            if (result) {
    ...
    const getDataByID = async () => {
        const result = await axios.get("http://127.0.0.1:8000/api/findPengadaan/" + idBarang);
        setPengadaan(result);
        setData(prevData => ({
            ...prevData,
            ...
            totalHargaBarang: result.data.results.quantity * result.data.results.hargaBarang
        }));
    ...
    return (
        <div className="bg-white w-[96%] mt-3  mb-[200px]  mx-auto p-3 rounded-lg">
            <div className="bg-white w-[96%] mt-3  mb-[200px]  mx-auto p-3 rounded-lg">
                {
                    pengadaan.length !== 0 ? (<div className="w-[95%] mx-auto h-[130vh] bg-white rounded-xl">
              ...
              <input type="file" id="ktp" name="ktp" className="hidden" />
                            ...
                            <div className="w-full justify-center mt-12 mb-12 flex items-center">
                                <button onClick={() => UpdatePengadaan()} className="bg-[#7B2CBF] px-3 py-1 w-[140px] rounded-md text-[#E5D5F2] font-abc">
                                    Simpan
                                </button>
                                <button
